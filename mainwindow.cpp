    ...
    : QMainWindow(parent)
{
    ...
    // Creating UI
    const QString iconPath = ":/LeagueScreenshot.ico";
    createActions();
    createTrayIcon(iconPath);

    // creating directory path browsing button
    infoLabel = new QLabel("Path to '\\League of Legends\\Screenshots' directory");
...
void MainWindow::updateUI()
{
    if(m_dirPath->isDirPathCorrect()){
        textColorPalette.setColor(QPalette::WindowText, Qt::darkGreen);
        pathCorrectionLabel->setText("(Correct)");
    }else{
        textColorPalette.setColor(QPalette::WindowText, Qt::darkRed);
...
void MainWindow::onEditingFinished()
{
    ...
    if(!m_dirPath->setDirPath(path_temp)){
        LogFile::instance() << "ERROR! On editing path, unable to set given directory: " << pathDirectoryEdit->text() << '\n';
...
{
    // LogFile::instance() << "Browsing directories.\n";
    ...
    if(directory.isEmpty()){
        // Directory browsing canceled
        return;
    }
    directory += '/';   // append at the end
    pathDirectoryEdit->setText(directory);

    if(!m_dirPath->setDirPath(directory)){
        LogFile::instance() << "ERROR! On browsing directories, unable to set given directory: " << pathDirectoryEdit->text() << '\n';
        QMessageBox::information(this, "Error on path editing.", "Unable to set, given directory path. Check given path again, if the folder exists.");
    }else{
        m_dirPath->saveDirPathToFile(this);
    }
